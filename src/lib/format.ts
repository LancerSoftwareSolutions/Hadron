...
export function formatSalary(
  job: { salary_min: number | null; salary_max: number | null; salary_currency: Currency },
  t: Dict,
): string {
  const { salary_min: min, salary_max: max, salary_currency: cur } = job
  if (min != null && max != null) {
    return min === max ? formatMoney(min, cur, t) : `${formatMoney(min, cur, t)} – ${formatMoney(max, cur, t)}`
  }
  if (min != null) return t.job.payFrom(formatMoney(min, cur, t))
  if (max != null) return t.job.payUpTo(formatMoney(max, cur, t))
