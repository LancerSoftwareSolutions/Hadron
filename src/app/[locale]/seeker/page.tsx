...
import { AREAS, CV_MAX_MB, SPOKEN_LANGUAGES } from '@/lib/constants'
        ...
        <form action={saveSeekerProfile} className="panel grid gap-4">
            ...
            <div className="flex flex-wrap gap-4">
              {SPOKEN_LANGUAGES.map((l) => (
                <label key={l} className="flex items-center gap-2 font-semibold">
                  <input type="checkbox" name="languages" value={l} defaultChecked={profile?.languages.includes(l)} className="size-4 accent-[#10233a]" />
                  {t.seeker.langs[l]}
                </label>
              ))}
            </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <SelectField
              label={t.seeker.area}
              name="area"
              options={AREAS.map((a) => ({ value: a, label: t.areas[a] }))}
              placeholder={t.seeker.chooseArea}
