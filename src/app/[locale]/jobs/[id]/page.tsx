        ...
        <aside className="grid content-start gap-6">
          <section className="panel">
            ...
            {!isOpen ? (
              ...
              <p className="mt-3 text-ink-soft">{user.role === 'admin' ? t.job.apply.adminNote : t.job.apply.employerNote}</p>
            ) : application ? (
              <div className="mt-3 grid gap-3">
                ...
                <details className="rounded-lg border border-line">
                  <summary className="cursor-pointer px-4 py-3 font-bold text-ink-soft">{t.job.apply.withdraw}</summary>
                  <div className="border-t border-line p-4">
                    <p className="font-semibold">{t.job.apply.withdrawWarning}</p>
                    <form action={withdrawApplication} className="mt-3">
                      <input type="hidden" name="locale" value={locale} />
                      <input type="hidden" name="listing_id" value={id} />
                      <input type="hidden" name="application_id" value={application.id} />
                      <SubmitButton className="btn btn-danger">{t.job.apply.withdrawConfirm}</SubmitButton>
                    </form>
                  </div>
                </details>
              </div>
