...
export default async function ApplicantsPage({ params, searchParams }: Props) {
  ...
  const { data } = await supabase
    .from('applications')
    .select(
      'id, listing_id, seeker_id, cover_note, status, created_at, seeker:profiles!applications_seeker_id_fkey(full_name, phone, seeker_profiles(headline, summary, skills, languages, experience_years, areas, cv_path))',
    )
    .eq('listing_id', id)
            ...
            <li key={a.id} className="panel">
              ...
              {d && (
                <dl className="mt-4 grid gap-2 text-[0.95rem]">
                  ...
                  {d.areas.length > 0 && <p className="text-ink-soft">{areasLabel(d.areas, t, locale, 8)}</p>}
                  {d.skills.length > 0 && (
                    <div>
