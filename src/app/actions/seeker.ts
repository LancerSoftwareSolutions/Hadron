...
export async function withdrawApplication(formData: FormData) {
  ...
  const listingId = str(formData, 'listing_id')
  const { data, error } = await supabase.from('applications').delete().eq('id', str(formData, 'application_id')).select('id')
  const back = listingId ? `/jobs/${listingId}` : '/seeker'
  if (error || !data || data.length === 0) go(locale, back, 'err', 'save_failed')
