...
- **Sign-in redirects** only ever go to a plain path on this site (`src/lib/safe-path.ts`). Confirmation links are always built from `NEXT_PUBLIC_SITE_URL`, never from a request header.
- **Email addresses** shown as links are limited to plain characters, so an address cannot add hidden options to a `mailto:` link.
- **File names:** uploads must be named `<user id>/<random id>.<ext>`, which blocks path tricks. CVs are private and only the person and the businesses they applied to can open them.
- **Abuse limits:** at most 100 open jobs per business, 50 applications per seeker per day, one waiting payment per job, and 4 admin alert emails per business per hour.
- **No payments on closed jobs**, and an admin cannot reopen a closed job by confirming a payment.
- **Vercel secrets:** the Supabase integration adds powerful keys (service role, JWT secret, database passwords) to the Vercel project. This site does not use them. Remove them if you do not need them elsewhere, so a mistake or leak cannot expose the whole database.
